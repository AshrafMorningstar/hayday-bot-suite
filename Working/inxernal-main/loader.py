...
# Ensure UTF-8 output on Windows consoles to prevent encoding errors on non-ASCII paths
...
HOT_GOT_OFF = 0x01506390  # clock_gettime in libg.so (called continuously on game/render thread)
...
def find_emulator(adb):
    try:
        ...
        pass
def find_asset_profiles(base_dir=None):
    if base_dir is None:
        base_dir = GAME_ASSETS_DIR
    profiles = []
    base = Path(base_dir)
    if not base.is_dir():
        return profiles
    for child in sorted(base.iterdir(), key=lambda p: p.name):
        ...
        if child.name == "update":
            profiles.append({"name": "Default (update)", "path": child})
        elif (child / "update").is_dir():
            profiles.append({"name": child.name, "path": child / "update"})
        elif (child / "data").is_dir() or (child / "fingerprint.json").is_file():
            profiles.append({"name": child.name, "path": child})
    return profiles


def timed_input_choice(default="1", timeout=5, valid_choices=("1", "2", "3", "4", "5", "6")):
    """Wait for user selection with a live countdown timer, defaulting to `default` after `timeout` seconds."""
    ...
    if sys.platform == "win32":
        try:
            ...
            while True:
                ...
                if remaining < 0:
                    sys.stdout.write(f"\r[*] Auto-selected [{default}] (timeout).                                     \n")
                    sys.stdout.flush()
                    return default
                if remaining != last_sec:
                    sys.stdout.write(f"\r Enter choice [1-6] (Auto-selecting [{default}] in {remaining}s... Press 1-6 or Enter): ")
                    sys.stdout.flush()
                    last_sec = remaining
                if msvcrt.kbhit():
                    ch = msvcrt.getwch()
    ...
    try:
        sys.stdout.write(f" Enter choice [1-6] (Press Enter for [{default}]): ")
        sys.stdout.flush()
        choice = input().strip()
...
def prompt_launch_mode(auto_mode=False):
    profiles = find_asset_profiles()
    chosen_path = profiles[0]["path"] if profiles else None

    # Check for specific profile selection via CLI
    if "--profile" in sys.argv:
        try:
            p_idx = sys.argv.index("--profile")
            if p_idx + 1 < len(sys.argv):
                req = sys.argv[p_idx + 1].strip().lower()
                for p in profiles:
                    if req in p["name"].lower():
                        chosen_path = p["path"]
                        print(f"[*] Selected profile via CLI: {p['name']}")
                        break
        except Exception:
            pass

    # 1. Check CLI flags for non-interactive automation
    if "--clean" in sys.argv or "--clean-launch" in sys.argv or os.environ.get("NX_CLEAN_LAUNCH") == "1":
        print("[*] Launch Mode CLI Override: Clean Launch (vanilla assets)")
        return "clean", None
    if "--apply-assets" in sys.argv or "--apply" in sys.argv:
        print("[*] Standalone Action: Apply Assets Now")
        return "apply_only", chosen_path
    if "--remove-assets" in sys.argv or "--remove" in sys.argv:
        print("[*] Standalone Action: Remove Assets Now")
        return "remove_only", None
    if "--editor" in sys.argv or "--edit-assets" in sys.argv:
        print("[*] Launching Live Asset Editor")
        return "editor", None
    if "--direct" in sys.argv or "--vanilla" in sys.argv or os.environ.get("NX_DIRECT_LAUNCH") == "1":
        print("[*] Launch Mode CLI Override: Direct Launch (existing device assets)")
        return "direct", None
    if "--upload" in sys.argv or "--upload-launch" in sys.argv or "--modded" in sys.argv or os.environ.get("NX_UPLOAD_ASSETS") == "1":
        print("[*] Launch Mode CLI Override: Re-Upload Assets & Launch")
        return "upload", chosen_path

    # If test or automated background loop, immediately select default upload without blocking
    if auto_mode or not sys.stdin.isatty():
        print(f"[*] Non-interactive/Auto mode: automatically uploading '{profiles[0]['name'] if profiles else 'default'}' assets.")
        return "upload", chosen_path

    print("\n" + "=" * 66)
    print("          HAY DAY ASSET CONTROLLER & LAUNCH OPTIONS")
    print("=" * 66)
    if profiles:
        print(" Available Modded Asset Packs:")
        for idx, p in enumerate(profiles, 1):
            try:
                file_count = sum(len(files) for _, _, files in os.walk(p["path"]))
                size_mb = sum(
                    sum(os.path.getsize(os.path.join(r, f)) for f in files)
                    for r, _, files in os.walk(p["path"])
                ) / (1024 * 1024)
                print(f"   [{idx}] {p['name']} ({file_count} files, {size_mb:.1f} MB)")
            except Exception:
                print(f"   [{idx}] {p['name']}")
    else:
        print(" (No custom asset packs found in com.supercell.hayday)")
    print("-" * 66)
    print(" Program Launch Actions:")
    print("   [1] Re-Upload Assets & Launch Cleanly (Sync latest files every time) [DEFAULT]")
    print("   [2] Clean Launch Without Assets       (Remove modded assets, vanilla start)")
    print("   [3] Direct Launch Without Touching    (Keep current device state and launch)")
    print()
    print(" Standalone Asset Management (No game start):")
    print("   [4] Apply / Upload Assets Now         (Push assets to LDPlayer and exit)")
    print("   [5] Remove / Wipe Assets Now          (Delete update assets from LDPlayer and exit)")
    print("   [6] Open Live Asset Editor & Tester   (Inspect/edit exceed assets & test)")
    print("=" * 66)

    choice = timed_input_choice(default="1", timeout=5, valid_choices=("1", "2", "3", "4", "5", "6"))
    ...
    if choice == "2":
        ...
        if profiles and len(profiles) > 1:
            try:
                sys.stdout.write(f" Select asset pack [1-{len(profiles)}] (Press Enter for 1): ")
                sys.stdout.flush()
                p_choice = timed_input_choice(default="1", timeout=4, valid_choices=[str(i) for i in range(1, len(profiles) + 1)])
                p_idx = (
                    int(p_choice) - 1
                    if p_choice.isdigit() and 1 <= int(p_choice) <= len(profiles)
                    else 0
                )
                chosen_path = profiles[p_idx]["path"]
            except Exception:
                chosen_path = profiles[0]["path"]
        return "upload", chosen_path
...
class NXRTHConsole:
    ...
    def _get_app_uid_gid(self):
        res = su_command(self.adb, self.device_id, f"stat -c %u:%g {APP_DATA_DIR}", check=False)
        out = (res.stdout or "").strip() if res else ""
        if re.match(r"^\d+:\d+$", out):
            return out
        return "10061:10061"

    def _inspect_remote_assets(self):
        res = su_command(self.adb, self.device_id, f"ls -1 {APP_UPDATE_DIR} 2>/dev/null", check=False)
        files = [f.strip() for f in (res.stdout or "").splitlines() if f.strip()] if res else []
    ...
    def upload_game_assets(self, profile_path=None):
        if not profile_path:
            profiles = find_asset_profiles(GAME_ASSETS_DIR)
            if not profiles:
                raise LoaderError(f"No asset profiles found in {GAME_ASSETS_DIR}")
            profile_path = profiles[0]["path"]
        profile_p = Path(profile_path).resolve()
        if not profile_p.is_dir():
            raise LoaderError(f"Asset directory does not exist: {profile_p}")

        print(f"[*] Ensuring {PACKAGE_NAME} is stopped before asset upload...")
        adb_cmd(self.adb, self.device_id, "shell", f"am force-stop {PACKAGE_NAME}", timeout=5)
        time.sleep(0.3)

        t0 = time.monotonic()
        items = list(profile_p.iterdir())
        file_count = sum(len(files) for _, _, files in os.walk(profile_p))
        profile_name = profile_p.parent.name if profile_p.name == "update" else profile_p.name
        print(f"[*] Packaging {file_count} asset files from '{profile_name}' into fast deployment stream...")
        ...
        try:
            ...
            tar_size_mb = os.path.getsize(tar_tmp) / (1024 * 1024)
            print(f"[*] Pushing asset bundle ({tar_size_mb:.2f} MB) to device staging...")
            stage_tar = "/data/local/tmp/.nxrth_update.tar"
            adb_checked(self.adb, self.device_id, "push", tar_tmp, stage_tar, timeout=60)

            uid_gid = self._get_app_uid_gid()
            print(f"[*] Unpacking assets into {APP_UPDATE_DIR} and setting permissions ({uid_gid}, 0777)...")
            extract_script = (
            ...
            su_command(self.adb, self.device_id, extract_script)
        finally:
        ...
        print(f"[+] All modded assets successfully uploaded & applied in {elapsed:.2f}s!")

    def clean_game_assets(self):
        print(f"[*] Ensuring {PACKAGE_NAME} is stopped before asset clean...")
        adb_cmd(self.adb, self.device_id, "shell", f"am force-stop {PACKAGE_NAME}", timeout=5)
        time.sleep(0.3)

        print(f"[*] Cleaning modded assets from {APP_UPDATE_DIR}...")
        clean_cmd = f"rm -rf {APP_UPDATE_DIR}/* {APP_UPDATE_DIR}"
        su_command(self.adb, self.device_id, clean_cmd, check=False)
        print(f"[+] Clean launch prepared: update assets directory cleared. Game will run vanilla.")

    def handle_pre_launch_assets(self, launch_mode="upload", asset_profile=None):
        if launch_mode == "clean":
            print("\n[*] ================= PRE-LAUNCH: CLEAN ASSETS =================")
            self.clean_game_assets()
        elif launch_mode == "upload":
            print("\n[*] ================= PRE-LAUNCH: RE-UPLOAD ASSETS =================")
            self.upload_game_assets(asset_profile)
        elif launch_mode == "apply_only":
            print("\n[*] ================= STANDALONE: APPLY ASSETS =================")
            self.upload_game_assets(asset_profile)
            print("[+] Done. Assets successfully applied to LDPlayer.")
            sys.exit(0)
        elif launch_mode == "remove_only":
            print("\n[*] ================= STANDALONE: REMOVE ASSETS =================")
            self.clean_game_assets()
            print("[+] Done. Update assets removed from LDPlayer (clean vanilla state).")
            sys.exit(0)
        elif launch_mode == "editor":
            print("\n[*] Launching Live Asset Editor...")
            subprocess.run([sys.executable, str(PROJECT_DIR / "asset_editor.py")])
            print("[*] Asset editor closed. Proceeding with game launch.")
        else:
            print("\n[*] ================= PRE-LAUNCH: DIRECT LAUNCH =================")
            print("[*] Launching game keeping current device assets as-is.")
    ...
    def _install_native_gate(self):
        ...
        self.nat_F = F
        # Read the clean prologue for the tick at F
        ...
        if not stolen or len(stolen) != 16 or not self._stolen_is_pi(stolen):
            ...
            if raw and len(raw) >= 16 and self._stolen_is_pi(bytes(raw)[:16]):
                stolen = bytes(raw)[:16]
        if not stolen or len(stolen) != 16 or not self._stolen_is_pi(stolen):
    ...
    def _field_ids(self, verbose=True):
        ...
        if n is None:
            # Fall back to background module heap scan or RPC walk:
            scan_ids, _, _, _ = self._native_scan_ids()
            if scan_ids:
                if verbose:
                    print(f"  (native container scan fallback: {len(scan_ids)} fields)")
                ids = scan_ids
            else:
                gm = self._read_u64(self.nat_mbox + 0x20) if getattr(self, "nat_mbox", None) else 0
                ids = self._enumerate_fields(gm or None)
                if ids and verbose:
                    print(f"  (field scan fallback: {len(ids)} fields)")
            if not ids:
                if verbose:
        ...
        if not ids:
            ...
            ids = self._enumerate_fields(gm or None)
            if ids and verbose:
                print("  (native container empty; used field scan fallback)")
    ...
    def prepare_assets(self):
        ...
        valid_digests = FRIDA_SHA256 if isinstance(FRIDA_SHA256, (tuple, list, set)) else (FRIDA_SHA256,)
        if server_digest not in valid_digests:
            raise LoaderError(
    ...
    def _flush_line_once(self, base, addr_abs, got_off=HOT_GOT_OFF):
    ...
    def _do_field_command(self, ctor_off, crop_id, name, explicit_ids=None):
        ...
        if explicit_ids is None:
            ids = self._enumerate_fields(self._read_u64(m + 0x08) or None)
            if not ids:
    ...
    def cmd_plant(self, args):
        if getattr(self, "nat_mbox", None) or self._native_module_base():
            if self._ensure_native_mbox():
                return self.cmd_nplant(args)
        if len(args) < 1:
    ...
    def _enumerate_field_objs(self, gamemode=None):
        ...
        # Fast path: direct memory scan for FIELD_VTABLE (finds all fields in 100ms)
        try:
            pat = " ".join(f"{b:02X}" for b in struct.pack("<Q", vt_abs))
            matches = self._rpc("scanmem", pat) or []
            if matches:
                objs = {}
                for m in matches:
                    a = int(m, 16)
    ...
    def cmd_lookup(self, args):
        """Search in-game global IDs database by name or ID.
        Usage: lookup <keyword>"""
        if not args:
            print("  Usage: lookup <name_or_keyword>")
            print("  Example: lookup wheat, lookup boat, lookup plank, lookup bakery")
            return
        query = " ".join(args).lower()
        db_path = PROJECT_DIR / "game_global_ids.json"
        if not db_path.is_file():
            print("  [!] Global IDs database not found. Run tools/extract_global_ids.py first.")
            return
        try:
            with open(db_path, "r", encoding="utf-8") as f:
                data = json.load(f)
        except Exception as e:
            print(f"  [!] Failed to load global IDs: {e}")
            return
        matches = []
        for cat, items in data.items():
            if cat == "metadata" or not isinstance(items, list):
                continue
            for it in items:
                name = str(it.get("name", "")).lower()
                gid = str(it.get("global_id", ""))
                if query in name or query == gid:
                    matches.append((cat, it))
        if not matches:
            print(f"  No entries found matching '{query}'.")
            return
        print(f"  Found {len(matches)} match(es) for '{query}':")
        for cat, it in matches[:25]:
            gid = it.get("global_id", "-")
            name = it.get("name", "")
            lvl = it.get("level", "-")
            cat_name = cat.replace("_", " ").title()
            extra = []
            if "grow_time_seconds" in it:
                extra.append(f"time={it['grow_time_seconds']}s")
            if "max_price" in it and it["max_price"]:
                extra.append(f"price={it['max_price']}")
            if "slots" in it:
                extra.append(f"slots={it['slots']}")
            extra_str = f" ({', '.join(extra)})" if extra else ""
    def cmd_assets(self, args):
        """List available local asset packs in com.supercell.hayday and device status."""
        profiles = find_asset_profiles()
        print(f"\n[*] Available asset packs in {GAME_ASSETS_DIR}:")
        for idx, p in enumerate(profiles, 1):
            file_count = sum(len(files) for _, _, files in os.walk(p['path']))
            size_mb = sum(
                sum(os.path.getsize(os.path.join(r, f)) for f in files)
            ...
            print(f"  [{idx}] {p['name']} ({file_count} files, {size_mb:.1f} MB) -> {p['path']}")
        st = self._inspect_remote_assets()
        print(f"[*] Remote device state: {st['state']} ({len(st['files'])} items in {APP_UPDATE_DIR})")

    def cmd_upload(self, args):
        """Upload custom assets from com.supercell.hayday to device."""
        ...
        if not matched and profiles:
            matched = profiles[0]["path"]
        if not matched:
            print(f"  [!] No asset profile found in {GAME_ASSETS_DIR}")
            return
        self.upload_game_assets(matched)

    def cmd_clean(self, args):
        """Clean custom modded assets from device (vanilla game state)."""
        self.clean_game_assets()

    def cmd_reload(self, args):
        """Hot-reload hook.js into the active live session without game restart."""
        if not self.session:
            print("  [!] Not attached to Gadget.")
            return
        try:
            code = SCRIPT_PATH.read_text(encoding="utf-8")
            if self.script is not None and not self.script.is_destroyed:
                try:
                    self.script.unload()
                except Exception:
                    pass
            ...
            self.script.on("message", self.on_message)
            self.script.load()
            print("  [+] hook.js hot-reloaded successfully in live game!")
        except Exception as e:
            print(f"  [!] Reload failed: {e}")

    def cmd_livedebug(self, args):
        """Interactive Live Debugging REPL. Apply code and test actions directly on the live game."""
        print("\n+-------------------------------------------------------------+")
        print("|                  INXERNAL LIVE DEBUG REPL                   |")
        print("+-------------------------------------------------------------+")
        print("| Actions:                                                    |")
        print("|   harvest                - trigger live harvest on fields   |")
        print("|   plant [crop]           - trigger live plant on all fields |")
        print("|   sell <slot> [cnt] [$$] - list item in roadside shop       |")
        print("|   fields                 - inspect live field IDs           |")
        print("|   loadnative             - load native engine (libmetrics)  |")
        print("|   nping                  - ping native mailbox              |")
        print("| UI & Touch:                                                 |")
        print("|   shop / openshop        - tap shop button on screen        |")
        print("|   tap <x> <y>            - tap screen coordinates           |")
        print("|   swipe <x1> <y1> <x2> <y2> [ms] - swipe screen             |")
        print("|   back                   - press back button (close modals) |")
        print("| Debug & Development:                                        |")
        print("|   lookup <keyword>       - search Global IDs database       |")
        print("|   reload                 - hot-reload hook.js live          |")
        print("|   eval <python_expr>     - evaluate Python expr on self     |")
        print("|   exec <python_code>     - execute Python statement live    |")
        print("|   rpc <method> [args...] - call Frida RPC export            |")
        print("|   exit / quit            - return to main console           |")
        print("+-------------------------------------------------------------+\n")
        ...
        while True:
            try:
                ...
                cmd, c_args = parts[0].lower(), parts[1:]

                if cmd == "help":
                    print("  Commands: harvest, plant, sell, fields, loadnative, nping, shop, tap, swipe, back, lookup, reload, eval, exec, rpc, exit")
                elif cmd == "harvest":
                    try:
                        self.cmd_nharvest([])
                    except Exception as e:
                        print(f"  [!] nharvest failed: {e}. Trying legacy harvest...")
                        self.cmd_harvest([])
                elif cmd == "plant":
                    crop = c_args[0] if c_args else "400001"
                    if not crop.isdigit():
                        db_path = PROJECT_DIR / "game_global_ids.json"
                        if db_path.is_file():
                            data = json.loads(db_path.read_text(encoding="utf-8"))
                            for it in data.get("crops_and_fields", []):
                                if crop.lower() in it.get("name", "").lower():
                                    crop = str(it["global_id"])
                                    print(f"  Resolved '{c_args[0]}' -> ID {crop} ({it.get('name')})")
                                    break
                    try:
                        self.cmd_nplant([crop])
                    except Exception as e:
                        print(f"  [!] nplant failed: {e}. Trying legacy plant...")
                        self.cmd_plant([crop])
                elif cmd == "sell":
                    # If item name was passed as 5th argument, resolve it
                    if len(c_args) >= 5 and not c_args[4].isdigit():
                        item_name = c_args[4].lower()
                        db_path = PROJECT_DIR / "game_global_ids.json"
                        if db_path.is_file():
                            data = json.loads(db_path.read_text(encoding="utf-8"))
                            for cat, items in data.items():
                                if isinstance(items, list):
                                    for it in items:
                                        if isinstance(it, dict) and item_name in it.get("name", "").lower():
                                            c_args[4] = str(it.get("global_id", it.get("id")))
                                            print(f"  Resolved item '{item_name}' -> ID {c_args[4]} ({it.get('name')})")
                                            break
                    self.cmd_nsell(c_args)
                elif cmd == "fields":
                    try:
                        self.cmd_nfields([])
                    except Exception:
                    ...
                    self.cmd_nping([])
                elif cmd in ("shop", "openshop"):
                    # Shop button (bottom-left cash register/awning icon)
                    print("  [+] Tapping Shop icon at (35, 440)...")
                    adb_cmd(self.adb, self.device_id, "shell", "input tap 35 440")
                elif cmd == "tap":
                    if len(c_args) < 2:
                        print("  Usage: tap <x> <y>")
                    else:
                        print(f"  [+] Tapping ({c_args[0]}, {c_args[1]})...")
                        adb_cmd(self.adb, self.device_id, "shell", f"input tap {c_args[0]} {c_args[1]}")
                elif cmd == "swipe":
                    if len(c_args) < 4:
                        print("  Usage: swipe <x1> <y1> <x2> <y2> [duration_ms]")
                    else:
                        dur = c_args[4] if len(c_args) > 4 else "300"
                        print(f"  [+] Swiping ({c_args[0]},{c_args[1]}) -> ({c_args[2]},{c_args[3]})...")
                        adb_cmd(self.adb, self.device_id, "shell", f"input swipe {c_args[0]} {c_args[1]} {c_args[2]} {c_args[3]} {dur}")
                elif cmd == "back":
                    print("  [+] Sending BACK keyevent...")
                    adb_cmd(self.adb, self.device_id, "shell", "input keyevent 4")
                elif cmd == "lookup":
                    self.cmd_lookup(c_args)
                elif cmd == "reload":
                    self.cmd_reload([])
                elif cmd == "eval":
                    expr = line[len("eval"):].strip()
                    try:
                        res = eval(expr, {"self": self, "console": self, "frida": frida, "struct": struct})
                        print(f"  => {res!r}")
                    except Exception as e:
                        print(f"  [!] Eval error: {e}")
                elif cmd == "exec":
                    stmt = line[len("exec"):].strip()
                    try:
                        exec(stmt, {"self": self, "console": self, "frida": frida, "struct": struct})
                        print("  [+] Exec completed.")
                    except Exception as e:
                        print(f"  [!] Exec error: {e}")
                elif cmd == "rpc":
                    if not c_args:
                        print("  Usage: rpc <method_name> [args...]")
                    else:
                        try:
                            res = self._rpc(c_args[0], *c_args[1:])
                            print(f"  RPC {c_args[0]} => {res}")
                        except Exception as e:
                            print(f"  [!] RPC error: {e}")
                else:
                    print(f"  Unknown command: {cmd}. Type 'help' for options.")
            except KeyboardInterrupt:
                print("\nExiting live debug mode.")
                break
            except Exception as e:
                print(f"  [!] Live debug error: {e}")
    ...
    def console_loop(self):
        commands = {
            ...
            "livedebug": self.cmd_livedebug,
            "live": self.cmd_livedebug,
        }
    ...
    def _control_result(self, line):
        ...
        if cmd == "assets_list":
            profiles = find_asset_profiles()
            res_list = [f"{p['name']}:{p['path']}" for p in profiles]
            return f"OK {len(profiles)}|" + "|".join(res_list)
        if cmd == "assets_upload":
            pname = args[0] if args else None
            profiles = find_asset_profiles()
            target = None
            if pname:
                for p in profiles:
            ...
            if not target and profiles:
                target = profiles[0]["path"]
            if not target:
                return "ERR no asset profiles found in com.supercell.hayday"
            try:
                self.upload_game_assets(target)
                return f"OK assets uploaded from {target.name}"
            except Exception as e:
                return f"ERR upload failed: {e}"
        if cmd == "assets_clean":
            try:
                self.clean_game_assets()
                return "OK assets cleaned"
            except Exception as e:
                return f"ERR clean failed: {e}"
        if cmd == "assets_status":
            st = self._inspect_remote_assets()
            return f"OK status={st['state']} items={','.join(st['files'])}"
    ...
    def auto_test_loop(self):
        print("\n[*] ================= AUTO TEST MODE =================")
        print("[*] Waiting 8s for game scene to stabilize...")
        time.sleep(8)

        print("\n[TEST 1/5] Checking ADB connection via control protocol...")
        adb_res = self._control_adb()
        print(f"  -> {adb_res}")

        print("\n[TEST 2/5] Loading native in-game engine (loadnative)...")
        try:
            self.cmd_loadnative([])
        except Exception as e:
            print(f"  [!] loadnative error: {e}")

        print("\n[TEST 3/5] Pinging native engine mailbox (nping)...")
        try:
            self.cmd_nping([])
        except Exception as e:
            print(f"  [!] nping error: {e}")

        print("\n[TEST 4/5] Inspecting field entities (nfields)...")
        try:
            self.cmd_nfields([])
        except Exception as e:
            print(f"  [!] nfields error: {e}")

        print("\n[TEST 5/5] Testing TCP control server status on 127.0.0.1:31350...")
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.connect(("127.0.0.1", 31350))
            s.sendall(b"status\n")
            rep = s.recv(1024).decode().strip()
            print(f"  -> Control server reply: {rep}")
            s.close()
        except Exception as e:
            print(f"  [!] Control socket check failed: {e}")

        print("\n[*] ================= AUTO TEST COMPLETED =================")
        return 0

    def run(self, auto_test=False, live_debug=False, launch_mode="upload", asset_profile=None):
        exit_code = 1
        try:
            self.setup_adb()
            self.handle_pre_launch_assets(launch_mode=launch_mode, asset_profile=asset_profile)
            self.prepare_assets()
...
def main():
    auto_test = "--auto" in sys.argv or "--test" in sys.argv or os.environ.get("NX_AUTO_TEST") == "1"
    live_debug = "--livedebug" in sys.argv or "--live" in sys.argv or os.environ.get("NX_LIVE_DEBUG") == "1"

    # Determine launch mode and asset profile from CLI flags or environment
    launch_mode = None
    asset_profile = None
    for arg in sys.argv[1:]:
        if arg == "--clean":
            ...
            launch_mode = "upload"
            if "=" in arg:
                asset_profile = arg.split("=", 1)[1]
        elif arg in ("--skip-upload", "--direct"):
            launch_mode = "direct"

    if not launch_mode:
        if os.environ.get("NX_CLEAN_LAUNCH") == "1":
            launch_mode = "clean"
        elif os.environ.get("NX_UPLOAD_ASSETS") == "1":
            launch_mode = "upload"
            asset_profile = os.environ.get("NX_ASSET_PROFILE")
        elif os.environ.get("NX_SKIP_UPLOAD") == "1":
            launch_mode = "direct"

    # In interactive CLI mode (not auto-test / live-debug), prompt user before launch
    if not launch_mode and not auto_test and not live_debug:
        try:
            launch_mode, asset_profile = prompt_launch_mode()
        except Exception as e:
            print(f"[*] Defaulting to modded upload launch mode: {e}")
            launch_mode = "upload"
    elif not launch_mode:
        # Default for auto-test / non-interactive
        launch_mode = "upload"
    ...
    while True:
        ...
        try:
            code = NXRTHConsole().run(
                ...
                launch_mode=launch_mode if attempt == 1 else "direct",
        ...
        if code in (0, 130) or auto_test or live_debug:
            return code
