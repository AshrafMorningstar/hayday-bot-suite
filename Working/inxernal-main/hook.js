...
function resolveTarget(allowEnumerate) {
    ...
    // Fast path: check /proc/self/maps directly
    try {
        var content = File.readAllText("/proc/self/maps");
        var lines = content.split("\n");
        for (var i = 0; i < lines.length; i++) {
            var line = lines[i];
            if (line.indexOf(TARGET_LIB) !== -1) {
                var dash = line.indexOf("-");
                if (dash !== -1) {
                    var basePtr = ptr("0x" + line.substring(0, dash));
                    if (isTargetElf(basePtr)) {
                        return acceptTarget(basePtr);
                    }
                }
            }
        }
...
function requireTarget() {
    if (!resolveTarget(true)) throw new Error(TARGET_LIB + " is not loaded");
    return target.base;
}
