...
`;
  ...
  function installReportGuardFor(loader) {
    ...
    try {
      if (loader) {
        const lstr = String(loader);
        if (lstr.includes("com.google") || lstr.includes("Cronet") || lstr.includes("chimera") || lstr.includes("app_chimera")) {
          return;
        }
      }
  ...
  var directGuardTimer = setInterval(() => {
    ...
    if (reportGuardInstalled || Date.now() - directGuardStartedAt > 5000) {
      clearInterval(directGuardTimer);
      if (!reportGuardInstalled && !guardStatus.installed)
