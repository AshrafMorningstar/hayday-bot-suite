...
def main():
    ...
    log("Gadget injected. Waiting for gadget listener to start...")
    # Wait for gadget to start listening (Houdini ARM emulation needs extra time)
    time.sleep(3.0)

    # 4. Connect to Gadget — extended timeout for ARM-on-x86 (Houdini)
    gadget_addr = f"127.0.0.1:{args.gadget_port}"
    log(f"Connecting to Gadget on {gadget_addr} (timeout 30s)...")
    gadget_dev = None
    session = None
    last_err = None
    ...
    while time.monotonic() < deadline:
        try:
            ...
            time.sleep(0.5)
    if session is None:
        raise RuntimeError(f"Failed to attach to Gadget after 30s: {last_err}")
    ...
    actual_pid = status.get("pid")
    if actual_pid != pid:
        # Android zygote always forks a new PID — this is expected and NOT an error.
        log(f"Note: Gadget PID is {actual_pid} (spawned: {pid}) — normal Android zygote fork.")
    if status.get("resumeComplete") is not False:
        log("Warning: resume barrier already complete — continuing anyway.")

    log("Initializing agent...")
    if script.exports_sync.init() is not True:
        raise RuntimeError("Agent init failed")

    # 9. Resume process — use actual (post-fork) PID; original spawned PID is gone after zygote fork
    log("Resuming process...")
    resume_pid = actual_pid if actual_pid else pid
    try:
        injector.resume(resume_pid)
    except Exception:
        # Fallback: try original spawned pid (may still work on some ROM versions)
        try:
            injector.resume(pid)
        except Exception as e:
            log(f"Resume warning: {e} — process may have self-resumed")

    # 10. Wait for resume barrier — extended timeout for Houdini ARM emulation
