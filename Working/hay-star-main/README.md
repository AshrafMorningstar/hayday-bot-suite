...
## 🧒 Super Easy Command & Shortcut Guide
### *Even a 7-Year-Old Child Can Use It!* 🎈

You can type **EITHER** the full command **OR** the easy 2-3 letter shortcut!
Works inside `hay-star.exe` (`mstar>`) AND from PowerShell / CMD!

### 📋 Full Command vs Short 2-Letter Shortcut Table

| What You Want To Do | Full Command | Easy 2-3 Letter Shortcut | What to Type (Example) |
|---|---|---|---|
| **🌾 Run Farm Loop (Single Account)** | `master_cycle` | `mc` | `mc` *(harvests, plants, sells, collects)* |
| **🔄 Run Farm Loop (Multi-Account)** | `master_rotate` | `mr` | `mr` *(switches accounts automatically)* |
| **🩹 Fix Stuck Screen / Reconnect** | `auto_heal` | `rc` | `rc` *(taps 'Reload game' & clears popups)* |
| **🎮 Open Master Menu Dashboard** | `dashboard` | `db` (or `dash`) | `db` *(interactive color menu)* |
| **🚀 Run All Systems Auto** | `auto` | `all` | `all` *(all commands fully automatic)* |
| **⚡ Load Native Engine** | `loadnative` | `ln` | `ln` *(loads ARM64 C++ engine)* |
| **🌱 Plant Crops on All Fields** | `nplant` | `np` | `np 400001` *(plants wheat everywhere)* |
| **🚜 Harvest Ready Crops** | `nharvest` | `nh` | `nh` *(harvests all fields instantly)* |
| **📦 Find Farm Fields** | `nfields` | `nf` | `nf` *(shows field count)* |
| **💰 Sell on Roadside Shop** | `nsell` | `ns` | `ns 0 10 36 1 400001` *(safe max price)* |
| **🔄 Built-In Farm Loop** | `nfarm` | `nfa` (or `nfrm`) | `nfa` *(loop with wait timer)* |
| **📍 Teleport Camera** | `jump` | `j` | `j shop` or `j farm` or `j animals` |
| **🔍 Search Any Item or Magic ID** | `search` | `s` | `s wheat` or `s bread` or `s 400001` |
| **📜 List All ID Families** | `ids` | `ids` | `ids` *(shows all item categories)* |
| **🧪 Run All 28 Diagnostic Tests** | `test` | `tst` | `tst` *(runs full test suite)* |
| **❓ Show Help** | `help` | `h` | `h` *(shows command list)* |
| **🚪 Quit / Exit** | `quit` | `q` | `q` *(exits program)* |

> 🌟 **Full Global IDs Catalog:** Check out our dedicated [ALL_GLOBAL_IDS_GUIDE.md](file:///c:/Users/Admin/Documents/hay-star-main/ALL_GLOBAL_IDS_GUIDE.md) for every single item number, formula, price, and ingredient in Hay Day!

---

## 🧸 How to Test Everything in 1 Minute!

### 1️⃣ Quick Test: Search for Any Item
Type this in your terminal:
```powershell
python launcher.py s wheat
```
*(Or in `hay-star.exe`, just type `s wheat`)*
You will instantly see the Global ID `400001` and its maximum safe price!

### 2️⃣ Quick Test: Auto-Heal & Popup Dismissal
Type this:
```powershell
python launcher.py rc
```
The bot immediately checks LDPlayer 9, dismisses any "Another device connected" or "Reload game" modal popups, and brings the game back to life!

### 3️⃣ Quick Test: Full 28-Test Certificate
Type this:
```powershell
python tests/test_all.py
```
Watch all 28 automated tests pass with `OK (28/28 tests passed - 100%)`!
